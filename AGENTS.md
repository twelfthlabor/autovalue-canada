...
- Python: use 3.11 (`/Library/Frameworks/Python.framework/Versions/3.11/bin/python3 -m venv`). That bin has no `python` binary and `python` is not on PATH here — invoke `python3.11` by absolute path, e.g. `PYTHONDONTWRITEBYTECODE=1 /Library/Frameworks/Python.framework/Versions/3.11/bin/python3.11 aws/check_parity.py`. `python3.13`/`3.14` have no pip and `requirements.txt` pins (numpy 2.1.3, pandas 2.2.2) lack 3.13 wheels — installs hang building from source. Never upgrade pins without checking 3.11 wheels first.
